  ...
  return (
    <div className="page-fqa">
      ...
      {questions.map((question) => (
        <>
            <div>
            <button className={`button-fqa ${question.expanded ? 'button-clicked' : ''}`}
            onClick={() => toggleAnswer(question.id)}>{question.question}</button>
            </div>

            <div className={question.expanded && 'div-answer'}>
                {question.expanded && <p className='answer'>{question.answer}</p>}
            </div>
